...
whitespace tokenizer, the Byte-Pair Encoding algorithm used by GPT-2,
...
regex pattern instead (see~\S\ref{sec:pretok}).
...
\begin{keyconcept}
  ...
  straightforward---just join with spaces. For BPE, decoding concatenates
  subword strings and removes the special end-of-word markers that BPE
  uses internally.
...
Byte-Pair Encoding starts from characters and iteratively merges the most
frequent adjacent pair, building up longer and longer subword tokens.
The result is a vocabulary that grows from individual characters to
common multi-character sequences, all driven by what the data demands.

\subsection{Pre-Tokenization\label{sec:pretok}}
...
characters exactly as described above. The resulting vocabulary consists
of the 256 byte-level base tokens plus the learned merge tokens plus
special tokens, yielding GPT-2's total vocabulary size of 50,257.
