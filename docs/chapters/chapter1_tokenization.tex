...
\begin{pseudocode}[SimpleTokenizer: encode]
class SimpleTokenizer:
    def __init__(self, vocab: dict[str, int]):
        self.vocab = vocab                          # token -> ID
        self.reverse_vocab = {v: k for k, v in vocab.items()}  # ID -> token
...
\begin{keyconcept}[Worked Example: BPE Training]
  ...
  Ties are broken arbitrarily---suppose we pick (l, o). We merge into ``lo'':
