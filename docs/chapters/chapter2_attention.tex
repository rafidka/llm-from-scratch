...
\section{Scaled Dot-Product Attention\label{sec:scaled-dot-product}}
...
def scaled_dot_product_attention(Q: Tensor, K: Tensor, V: Tensor) -> Tensor:
    ...
    attn_weights = dropout(attn_weights, p=dropout_p, training=is_training)  # is_training=False during inference
...
lookup. The database analogy from Section~\ref{sec:scaled-dot-product} applies here too, but now we can
